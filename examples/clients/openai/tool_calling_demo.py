#!/usr/bin/env python3
"""
OpenAI Client Tool Calling Demo

This example demonstrates how the OpenAI client integrates with
MCP tools using the responses API for function calling.
"""
...
async def tool_calling_demo():
    """Demonstrate OpenAI client with MCP tool integration"""
    
    # Configuration for OpenAI client with tool calling
    config = {
        "LLM_API_KEY": os.getenv("OPENAI_API_KEY", "your-openai-api-key-here"),
        "LLM_MODEL": "gpt-4o-mini",  # Good for tool calling
        "max_tokens": 2000,
    ...
    client = OpenAiClient.with_in_memory_store(config, user_id="tool_demo_user")
    
    try:
        print("🔧 OpenAI Client Tool Calling Demo")
        print("=" * 40)
...
async def streaming_with_tools_demo():
    ...
    config = {
        ...
        "max_tokens": 1500,
        "system_prompt": "You are a real-time financial assistant. Use tools to get current data and stream your analysis.",
        "agentServers": {
            "viaNexus": {
                "server_url": "https://api.vianexus.com",
                "server_port": 443,
